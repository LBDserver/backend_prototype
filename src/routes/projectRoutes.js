...
router.get('/:projectName/graphs', authenticate, checkAccess, getNamedGraph) // get single named graphs, as TTL
...
router.patch('/:projectName/graphs', authenticate, checkAccess, deleteNamedGraph) // SPARQL UPDATE on a specific named graph => use sparql update on /:projectName/query
router.delete('/:projectName/graphs', authenticate, checkAccess, deleteNamedGraph) // delete named graph by referring to its URI

router.post('/:projectName/files/upload', authenticate, checkAccess, upload.fields([{name: 'file'}, {name: 'acl'}]), uploadDocumentToProject) // upload a document to the document store. Objects are be linked to the project metadata, but can be referenced in named graphs. 
